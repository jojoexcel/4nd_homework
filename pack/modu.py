    ...
    if all_data:
        ...
        for row in all_data:
            if len(row) == len(columns):
                formatted_row = "| " + " | ".join(pad_to_width(str(col), col_settings["width"]) if col_settings["name"] in columns else pad_to_width(str(col), col_settings["width2"]) for col, col_settings in zip(row, table_config["columns_set"])) + " |"
                print(formatted_row)
            else:
                print("資料欄位數量與設定不符")
